...
set BACKUP
set SELECT="%adalid_dir%\source\documents\sql\postgresql\select-meta-java-count.sql"
set SELECT
...
echo %PGEXE1% %PGCOPT% %PGOPT1% --file %BACKUP%
echo %PGEXE1% %PGCOPT% %PGOPT1% --file %BACKUP% 1>>%PGLOG1%
call %PGEXE1% %PGCOPT% %PGOPT1% --file %BACKUP% 1>>%PGLOG1% 2>&1
echo %DATE%-%TIME% 1>>%PGLOG1%
echo.
echo %PGEXE2% %PGCOPT% %PGOPT2% --dbname %DBNAME% & echo.
call %PGEXE2% %PGCOPT% %PGOPT2% --dbname %DBNAME%
echo %DATE%-%TIME% 1>>%PGLOG1%
echo %PGEXE1% %PGCOPT% %PGOPT1% --file %SELECT% --dbname %DBNAME%
echo %PGEXE1% %PGCOPT% %PGOPT1% --file %SELECT% --dbname %DBNAME% 1>>%PGLOG1%
call %PGEXE1% %PGCOPT% %PGOPT1% --file %SELECT% --dbname %DBNAME% 1>>%PGLOG1% 2>&1
echo %DATE%-%TIME% 1>>%PGLOG1%
echo.
set siono=S

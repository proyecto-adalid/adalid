 ...
 * PCX, PNG, IFF, RAS, PBM, PGM, PPM and PSD files (or input streams).
 * <p>
 ...
 * <li><strong>2004-11-30</strong> Bug fix: recognizing progressive GIFs (interlaced in GIF terminology) did not work (thanks to Franz Jeitler for
 * pointing this out). Now it should work, but only if the number of images is determined. This is because information on interlacing is stored in a
 * local image header. In theory, different images could be stored interlaced and non-interlaced in one file. However, I think that's unlikely. Right
 * now, the last image in the GIF file that is examined by ImageInfo is used for the "progressive" status.</li>
 * <li><strong>2005-01-02</strong> Some code clean up (unused methods and variables commented out, missing javadoc comments, etc.). Thanks to George
 * Sexton for a long list. Removed usage of Boolean.toString because it's a Java 1.4+ feature (thanks to Gregor Dupont). Changed delimiter character
 * in compact output from semicolon to tabulator (for better integration with cut(1) and other Unix tools). Added some points to the <a
 * href="http://schmidt.devlib.org/image-info/index.html#knownissues">'Known issues' section of the website</a>. Released as 1.6.</li>
 * <li><strong>2005-07-26</strong> Removed code to identify Flash (SWF) files. Has repeatedly led to problems and support requests, and I don't know
 * the format and don't have the time and interest to fix it myself. I repeatedly included fixes by others which didn't work for some people. I give
 * up on SWF. Please do not contact me about it anymore. Set package of ImageInfo class to org.devlib.schmidt.imageinfo (a package was repeatedly
 * requested by some users). Released as 1.7.</li>
 * <li><strong>2006-02-23</strong> Removed Flash helper methods which weren't used elsewhere. Updated skip method which tries "read" whenever
 * "skip(Bytes)" returns a result of 0. The old method didn't work with certain input stream types on truncated data streams. Thanks to Martin Leidig
 * for reporting this and sending in code. Released as 1.8.</li>
 * <li><strong>2006-11-13</strong> Removed check that made ImageInfo report JPEG APPx markers smaller than 14 bytes as files in unknown format. Such
 * JPEGs seem to be generated by Google's Picasa application. First reported with fix by Karl von Randow. Released as 1.9.</li>
 * </ul>
 *
...
public class ImageInfo {
     ...
     * than one image are animations). It is determined whether the GIF stream is interlaced (see {@link #isProgressive()}).
     */
...
//
    /**
     * Call this method after you have provided an input stream or file using {@link #setInput(InputStream)} or {@link #setInput(DataInput)}. If true
     * is returned, the file format was known and information on the file's content can be retrieved using the various getXyz methods.
     *
...
//
    ...
    // <editor-fold defaultstate="collapsed" desc="comments">
    /**
     * If {@link #check()} was successful and {@link #setCollectComments(boolean)} was called with <code>true</code> as argument, returns the number
     * of comments retrieved from the input image stream / file. Any number &gt;= 0 and smaller than this number of comments is then a valid argument
     * for the {@link #getComment(int)} method.
     *
...
//
    /**
     * Returns the number of images in the examined file. Assumes that <code>setDetermineImageNumber(true);</code> was called before a successful call
     * to {@link #check()}. This value can currently be only different from <code>1</code> for GIF images.
     *
    ...
    /**
     * Returns the physical height of this image in dots per inch (dpi). Assumes that {@link #check()} was successful. Returns <code>-1</code> on
     * failure.
     *
    ...
    /**
     * Returns the physical width of an image in inches, or <code>-1.0f</code> if width information is not available. Assumes that {@link #check} has
...
//                  System.out.print(name + ";");
    ...
    private static void printCompact(String sourceName, ImageInfo imageInfo) {
        ...
        System.out.println(sourceName
            + SEP + imageInfo.getFormatName()
            + SEP + imageInfo.getMimeType()
            + SEP + imageInfo.getWidth()
            + SEP + imageInfo.getHeight()
            + SEP + imageInfo.getBitsPerPixel()
            + SEP + imageInfo.getNumberOfImages()
            + SEP + imageInfo.getPhysicalWidthDpi()
            + SEP + imageInfo.getPhysicalHeightDpi()
            + SEP + imageInfo.getPhysicalWidthInch()
            + SEP + imageInfo.getPhysicalHeightInch()
            + SEP + imageInfo.isProgressive());
    }
...
//      }
    ...
    /**
     * Specify whether textual comments are supposed to be extracted from input. Default is <code>false</code>. If enabled, comments will be added to
     * an internal list.
     *
    ...
    /**
     * Specify whether the number of images in a file is to be determined - default is <code>false</code>. This is a special option because some file
     * formats require running over the entire file to find out the number of images, a rather time-consuming task. Not all file formats support more
     * than one image. If this method is called with <code>true</code> as argument, the actual number of images can be queried via
